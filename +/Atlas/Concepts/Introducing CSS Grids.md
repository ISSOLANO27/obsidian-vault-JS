...
The **advantage** of using positive and negative gridline numbers is that **you can always reference both the first gridlines and the last gridline** no matter the size of the grid. *This will become important later when placing items at specific location within the grid or sizing those items to cover multiple rows and columns..*
